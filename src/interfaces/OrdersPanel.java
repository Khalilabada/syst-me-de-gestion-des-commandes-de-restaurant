...
public class OrdersPanel extends JPanel {
    ...
    private JButton activeFilterButton = null; // Pour suivre le bouton actif
    ...
    public OrdersPanel(GerantDashboard parent) {
     ...
     // Bouton "Tous"
        ...
        allButton.addActionListener(e -> setActiveFilterButton(allButton, null));

        // Bouton "Livraison"
        JButton deliveryButton = createFilterButton("Livraison");
        deliveryButton.addActionListener(e -> setActiveFilterButton(deliveryButton, "livraison"));

        // Bouton "Sur place"
        JButton onSiteButton = createFilterButton("Sur place");
        onSiteButton.addActionListener(e -> setActiveFilterButton(onSiteButton, "sur_place"));

        // Bouton "À emporter"
        JButton takeawayButton = createFilterButton("À emporter");
        takeawayButton.addActionListener(e -> setActiveFilterButton(takeawayButton, "a_emporter"));

        // Activer le bouton "Tous" par défaut
        setActiveFilterButton(allButton, null);
    ...
    private JButton createFilterButton(String text) {
        ...
        button.getModel().addChangeListener(e -> {
            if (button.getModel().isRollover() && button != activeFilterButton) {
                button.repaint();
            }
        });
    ...
    private void setActiveFilterButton(JButton button, String type) {
        // Réinitialiser le bouton précédemment actif
        if (activeFilterButton != null) {
            activeFilterButton.setBackground(secondaryColor);
            activeFilterButton.setForeground(textColor);
            activeFilterButton.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(primaryColor.darker(), 1),
                new EmptyBorder(8, 20, 8, 20)
            ));
        }
        ...
        if (activeFilterButton != null) {
            activeFilterButton.setBackground(primaryColor);
            activeFilterButton.setForeground(Color.WHITE);
            activeFilterButton.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(primaryColor.darker(), 1),
                new EmptyBorder(8, 20, 8, 20)
            ));
    ...
    private void refreshCommandes() {
        if (activeFilterButton != null) {
            String text = activeFilterButton.getText();
            if (text.equals("Tous")) {
                loadAllCommandes();
            } else if (text.equals("Livraison")) {
                filterCommandesByType("livraison");
            } else if (text.equals("Sur place")) {
                filterCommandesByType("sur_place");
            } else if (text.equals("À emporter")) {
                filterCommandesByType("a_emporter");
            }
        } else {
            loadAllCommandes();
        }
