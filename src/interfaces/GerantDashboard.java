...
public class GerantDashboard extends JFrame {
    ...
    private void initUI() {
        ...
        // Add tabs with icons
        tabbedPane.addTab("Tableau de bord", new StatsPanel(this));
        tabbedPane.addTab("Clients", new ClientsPanel(this));
        tabbedPane.addTab("Menu", new MenuPanel(this));
        tabbedPane.addTab("Commandes", new OrdersPanel(this));
    ...
    public void refreshClientTable() {
        SwingUtilities.invokeLater(() -> {
            // Get the main panel (first component added to content pane)
            JPanel mainPanel = (JPanel) getContentPane().getComponent(0);
            
            // Get the content panel which contains the tabbed pane
            JPanel contentPanel = (JPanel) mainPanel.getComponent(1); // BorderLayout.CENTER
            
            // Find the JTabbedPane in the content panel
            for (Component comp : contentPanel.getComponents()) {
                if (comp instanceof JTabbedPane) {
                    JTabbedPane tabbedPane = (JTabbedPane) comp;
                    
                    // Now search for the ClientsPanel in the tabbed pane
                    for (int i = 0; i < tabbedPane.getTabCount(); i++) {
                        Component tabComponent = tabbedPane.getComponentAt(i);
                        if (tabComponent instanceof ClientsPanel) {
                            ((ClientsPanel) tabComponent).refreshTable();
                            return; // Found and refreshed, exit
                        }
                    }
                }
            }
